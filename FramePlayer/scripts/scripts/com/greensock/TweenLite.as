package com.greensock
{
   ...
   public class TweenLite extends TweenCore
   {
      ...
      override public function renderTime(param1:Number, param2:Boolean = false, param3:Boolean = false) : void
      {
         ...
         {
            init();
            if(!_loc6_ && Boolean(this.cachedTime))
            {
      ...
      protected function init() : void
      {
         ...
         {
            if(!(_loc4_ in _reservedProps && !(_loc4_ == "timeScale" && this.target is TweenCore)))
            {
               if(_loc4_ in plugins && Boolean((_loc6_ = new (plugins[_loc4_] as Class)()).onInitTween(this.target,this.vars[_loc4_],this)))
               {
                  this.cachedPT1 = new PropTween(_loc6_,"changeFactor",0,1,_loc6_.overwriteProps.length == 1 ? String(_loc6_.overwriteProps[0]) : "_MULTIPLE_",true,this.cachedPT1);
                  ...
                  if(_loc6_.priority)
                  {
                     this.cachedPT1.priority = _loc6_.priority;
                     _loc5_ = true;
                  }
                  if(Boolean(_loc6_.onDisable) || Boolean(_loc6_.onEnable))
                  {
               ...
               {
                  this.cachedPT1 = new PropTween(this.target,_loc4_,Number(this.target[_loc4_]),typeof this.vars[_loc4_] == "number" ? Number(this.vars[_loc4_]) - this.target[_loc4_] : Number(this.vars[_loc4_]),_loc4_,false,this.cachedPT1);
                  this.propTweenLookup[_loc4_] = this.cachedPT1;
               }
            }
         }
         if(_loc5_)
         ...
         {
            _loc1_ = this.cachedPT1;
            while(_loc1_)
            {
               _loc1_.start += _loc1_.change;
               _loc1_.change = -_loc1_.change;
         ...
         if(_overwrite > 1 && this.cachedPT1 && (_loc2_ = masterList[this.target]) && _loc2_.length > 1)
         {
      ...
      public function killVars(param1:Object, param2:Boolean = true) : Boolean
      {
         ...
         {
            if(_loc5_ in propTweenLookup)
            {
               _loc4_ = propTweenLookup[_loc5_];
               if(_loc4_.isPlugin && _loc4_.name == "_MULTIPLE_")
               {
                  ...
                  if(_loc5_ != _loc4_.target.propName || _loc4_.name == "")
                  {
                     delete propTweenLookup[_loc5_];
                  }
               }
